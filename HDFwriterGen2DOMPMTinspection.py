...
# format of Gen2DOM PMT measurement
'''
X = 1 or 2
...
from DAQscripts.general.HDFwriterGen2DOMPMTinspection import load_dict 
...
class HDFwriterGen2DOMPMTinspection():
    ...
    def fit_v0(self, tstart=10, tend=40):
        ...
        for i in range(self.Nwfm-11):  #limit
            if(i%100==0):
...
class HDFwriterGen2DOMPMTinspection_Ex():
    ...
    def __init__(self, Nsample_max, Nwfm_init = 0):
        ...
        # signal peak timing window
        self.t_sig_start = 9  #ExTrig
        self.t_sig_stop  = 32  #ExTrig
    ...
    def fit_v0(self, tstart=5, tend=32):   #ExTrig
        ...
        for i in range(self.Nwfm):
            ...
            if(debug):
                ...
                print("charge     = ", self.data_charge_ch1[i])
                t = np.linspace(0, 100, 1001)
                plt.plot(x, y, "o",label = 'ch1 data')
                plt.plot(t, self.MakinoFit_20230711(t, popt[0], popt[1]),label = 'ch1 fitting')
            ...
            #p0 = [self.data_time_ch2[i]-self.tpeak, self.data_peak_ch2[i]]
            p0 = [self.data_time_ch2[i], self.data_peak_ch2[i]]
            try:
                popt, pcov = curve_fit(self.MakinoFit_20230711, x, y, p0=p0, maxfev=10000)
                self.data_time_fit_ch2[i]     = -popt[0]
                self.data_charge_fit_ch2[i]   = -popt[1]*self.norm
            ...
            if(debug):
                t = np.linspace(0, 100, 1001)
                plt.plot(x, y, "o",label = 'ch2 data')
                plt.plot(t, self.MakinoFit_20230711(t, popt[0], popt[1]),label = 'ch2 fitting')
                plt.xlim(tstart, tend)
                plt.xlabel('bins/17ns', fontsize=20)
