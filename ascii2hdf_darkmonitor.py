...
if(__name__ == "__main__"):
    ...
    conversion_ch1 = 2.0/4096/1670.0/60e6*1e12 # pC/1 ADC sample calibrated by micro base
    ...
    for n in range(Nfile):
        ...
        for line in f:
            ...
            if readstate==WAIT_FOR_HIT:
                ...
                if len(tokens)!=1:
                    ...
                    try:
                        temperature    = np.float32(tokens[0])
                        readstate=WAIT_FOR_HIT
                    except:
                        readstate=WAIT_FOR_HIT #unexpected input 
                continue
                ...
                for hexvalue in tokens:
                    try:
                        ...
                        readstate = WAIT_FOR_HIT #unexpected value
                        continue
        ...
        adcs_ch1 = np.empty((Nwfm, Nsample_max), dtype=np.uint16)
        adcs_ch2 = np.empty((Nwfm, Nsample_max), dtype=np.uint16)
        
        for i, adc_ch1, adc_ch2 in zip(range(len(adcs_ch1_spool)), adcs_ch1_spool, adcs_ch2_spool):
            adcs_ch1[i] = adc_ch1
            adcs_ch2[i] = adc_ch2
        end = time.time()
