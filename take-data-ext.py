...
if len(sys.argv)!=6:
	print("Usage:")
	print("python take-spe-data.py port outfile runtime PMTID dac")
	sys.exit(1)
...
try:
	...
	print(f"Failed to open output file \"{outname}\", exiting.")
	sys.exit(1)
...
#subprocess.run(["python", "draw_wf.py",outname])

subprocess.run(["python", "ascii2hdf_ExTrig.py",outname,hdfname,PMTID])
#subprocess.run(["python", "ascii2hdf.py",outname,hdfname,PMTID])

subprocess.run(["python", "read_pdf_ExTrig.py",hdfname])
